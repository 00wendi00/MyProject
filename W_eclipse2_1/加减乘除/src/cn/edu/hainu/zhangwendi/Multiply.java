...
public class Multiply
{
	...
	String[] num1 = new String[10000], 
	         ...
	         num3 = new String[20000];
	public  String[] multiply(String[] num10,String[] num20)
	{	
       ...
       {
    	   for(int p = s1+s2-1;p>=0;p--)
    	   {
    		   add3[i+p+1] += add1[i]*add2[p];
    		   if(add3[i+p+1]>=10)
    		   {
    			   add3[i+p] += add3[i+p+1]/10;
    			   add3[i+p+1] = (add3[i+p+1]%10);   			   
    		   }
    	   }
       }           
       for(int i=0;i<=2*s1-2;i++)
       {
    	   f9=i+1;
    	   if(add3[i]>0)
    	   {
       ...
       for(int i = f9;i<s1*2;i++)
    	   num3[i] = Integer.toString(add3[i]); 
       num3[s1*2] = ".";
       for(int i=s1*2+1;i<s1*2+s2*2+1;i++)
